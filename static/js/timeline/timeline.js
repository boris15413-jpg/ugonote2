...
(function(G){
  ...
  TL.play = async () => {
    ...
    if (G.Audio && G.Audio.bgmBuffer) {
      // Play from current frame with range support
    ...
    function tick(now) {
      ...
      if (fi >= S.frames.length) {
        if (S.loopPlay) {
          ...
          // Restart BGM from beginning if looping
