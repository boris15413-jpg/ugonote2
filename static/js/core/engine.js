...
!(function (G) {
  ...
  ((E.getLayerCanvas = (l) => R.canvases[layerMap[l]]),
    ...
    (E.getCache = (fid) => (
      S.fc.has(fid) || S.fc.set(fid, { A: null, B: null, C: null, P: null }),
      S.fc.get(fid)
    )),
    (E.touchLRU = (fid) => {
      const idx = S.lruOrder.indexOf(fid);
      (idx >= 0 && S.lruOrder.splice(idx, 1), S.lruOrder.push(fid));
    }),
    (E.snapToCache = () => {
      const dpr = C.DPR,
        c = E.getCache(E.curId());
      (["A", "B", "C"].forEach((l) => {
        const rctx = E.getLayerCtx(l);
        c[l] = rctx.getImageData(0, 0, S.CW * dpr, S.CH * dpr);
      }),
        (c.P = E.getLayerCtx("Photo").getImageData(
          0,
          0,
          S.CW * dpr,
          S.CH * dpr,
    ...
    (E.markDirty = (fid, layer) => {
      (S.dirtyIds.add(fid),
        S.dirtyLayers.has(fid) || S.dirtyLayers.set(fid, new Set()),
        S.dirtyLayers.get(fid).add(layer),
        (S.modified = !0));
    }),
    (E.markAllDirty = (fid) => {
      ["A", "B", "C", "P"].forEach((l) => E.markDirty(fid, l));
    }),
    (E.cacheToCanvas = (fid) => {
      ...
      ([
        ...
        (ctx.save(),
          ...
          c && c[k] && ctx.putImageData(c[k], 0, 0),
          ctx.restore(),
    ...
    (E.redrawVectorLayers = (fid) => {
      ...
      ["A", "B", "C"].forEach((l) => {
        ...
        vctx &&
          ...
          G.VectorPaths &&
            G.VectorPaths.renderAllPaths(vctx, fid, l, S.CW, S.CH));
      });
    }),
    (E.redrawSingleVectorLayer = (fid, layer) => {
      const dpr = C.DPR,
        vctx = R.getVectorCtx(layer);
      vctx &&
        (vctx.save(),
        vctx.setTransform(1, 0, 0, 1, 0, 0),
        vctx.clearRect(0, 0, S.CW * dpr, S.CH * dpr),
        ...
        vctx.setTransform(dpr, 0, 0, dpr, 0, 0),
        G.VectorPaths &&
          G.VectorPaths.renderAllPaths(vctx, fid, layer, S.CW, S.CH));
    }),
    (E._loadingCache = new Map()),
    (E.ensureCached = async (fid) => {
      if (S.fc.has(fid)) return (E.touchLRU(fid), S.fc.get(fid));
      if (E._loadingCache.has(fid)) return await E._loadingCache.get(fid);
      const p = (async () => {
        const c = { A: null, B: null, C: null, P: null };
        if (G.Storage)
          for (const l of ["A", "B", "C", "P"])
            try {
              c[l] = await G.Storage.loadLayer(fid, l, S.CW, S.CH);
            } catch (e) {}
        return (
          S.fc.set(fid, c),
          E.touchLRU(fid),
          E.evictLRU(),
          E._loadingCache.delete(fid),
          c
        );
      })();
      return (E._loadingCache.set(fid, p), await p);
    }),
    (E.evictLRU = async () => {
      for (; S.lruOrder.length > C.LRU_SIZE; ) {
        ...
        if (oldest !== E.curId()) {
          if (S.dirtyIds.has(oldest) && G.Storage) {
            ...
            if (c) {
              const ls =
                S.dirtyLayers.get(oldest) || new Set(["A", "B", "C", "P"]);
              for (const l of ls)
                try {
                  await G.Storage.saveLayer(oldest, l, c[l]);
                } catch (e) {}
              (S.dirtyIds.delete(oldest), S.dirtyLayers.delete(oldest));
            }
          }
          S.fc.delete(oldest);
        } else S.lruOrder.push(oldest);
      }
    }),
    (E.evictFar = () => {
      E.evictLRU();
    ...
    (E.flattenLayerToRaster = (fid, layer) => {
      ...
      if (
        ...
        G.VectorPaths.getFramePaths(fid, layer).length = 0;
      }
    }),
    (E.fitView = () => {
      const v = U.$("viewport");
      if (!v) return;
      const aw = v.clientWidth - 24,
        ah = v.clientHeight - 24;
    ...
    (E.updateTransform = () => {
      ...
      const zd = U.$("zoomDisp");
      zd && (zd.textContent = Math.round(100 * S.zoom) + "%");
    }),
    (E.zoomAtPoint = (factor, screenX, screenY) => {
      const v = U.$("viewport");
      if (!v) return;
      const vr = v.getBoundingClientRect(),
        cx = screenX - vr.left - vr.width / 2,
        cy = screenY - vr.top - vr.height / 2,
        oldZoom = S.zoom;
    ...
    (E.updateLayerZOrder = () => {
      ...
      (photo && (photo.style.zIndex = 2),
        S.layerOrder.forEach((l, i) => {
          const rc = R.getRasterCanvas(l),
            vc = R.getVectorCanvas(l);
          (rc && (rc.style.zIndex = 3 + 2 * i),
            vc && (vc.style.zIndex = 4 + 2 * i));
        }));
    }),
    (E.updateOnion = () => {
      const ctx = R.contexts.onC;
      if (!ctx) return;
      const dpr = C.DPR;
      if (
        (ctx.save(),
...
const tmp = document.createElement("canvas");
    ...
    (E.updateGrid = () => {
      const ctx = R.contexts.gridC;
      if (!ctx) return;
      if ((ctx.clearRect(0, 0, S.CW, S.CH), !S.grid)) return;
      const gs = S.pixelMode ? S.pixelSize : S.gridSize;
      ((ctx.strokeStyle = "rgba(0,0,0,.1)"), (ctx.lineWidth = 0.5));
      for (let x = gs; x < S.CW; x += gs)
        (ctx.beginPath(), ctx.moveTo(x, 0), ctx.lineTo(x, S.CH), ctx.stroke());
      for (let y = gs; y < S.CH; y += gs)
        (ctx.beginPath(), ctx.moveTo(0, y), ctx.lineTo(S.CW, y), ctx.stroke());
      ((ctx.strokeStyle = "rgba(255,0,0,.12)"),
        (ctx.lineWidth = 1),
        ctx.beginPath(),
        ctx.moveTo(S.CW / 2, 0),
        ctx.lineTo(S.CW / 2, S.CH),
        ctx.stroke(),
        ctx.beginPath(),
        ctx.moveTo(0, S.CH / 2),
        ctx.lineTo(S.CW, S.CH / 2),
        ctx.stroke());
    }));
  let undoSnap = null,
    undoVectorSnap = null;
  ((E.pushUndo = () => {
    ...
    (E.commitUndo = () => {
      if (!undoSnap) return;
      const dpr = C.DPR,
        after = E.curX().getImageData(0, 0, S.CW * dpr, S.CH * dpr),
        w = S.CW * dpr,
        h = S.CH * dpr,
        od = undoSnap.data,
        nd = after.data;
      ...
      if (G.VectorPaths && undoVectorSnap) {
        ...
        JSON.stringify(undoVectorSnap) !== JSON.stringify(currentPaths) &&
          ((vectorChanged = !0),
          (vOp = undoVectorSnap),
          (vNp = JSON.parse(JSON.stringify(currentPaths))));
      }
      if (
        ((undoSnap = null), (undoVectorSnap = null), maxX < 0 && !vectorChanged)
      )
        return;
      const dw = maxX >= 0 ? maxX - minX + 1 : 0,
        dh = maxY >= 0 ? maxY - minY + 1 : 0,
        op = new Uint8ClampedArray(dw * dh * 4),
        np = new Uint8ClampedArray(dw * dh * 4);
      if (dw > 0 && dh > 0)
        for (let y = 0; y < dh; y++)
          for (let x = 0; x < dw; x++) {
            const si = 4 * ((minY + y) * w + (minX + x)),
              di = 4 * (y * dw + x);
            ((op[di] = od[si]),
              (op[di + 1] = od[si + 1]),
              (op[di + 2] = od[si + 2]),
      ...
      (S.undoStack.push({
        ...
        S.undoStack.length > S.maxUndo && S.undoStack.shift(),
        (S.redoStack = []));
    }));
  const applyPatch = (entry, patch, vPatch) => {
    const ctx =
      "Photo" === entry.l ? E.getLayerCtx("Photo") : E.getLayerCtx(entry.l);
    if (entry.fid === E.curId()) {
      if (entry.w > 0 && entry.h > 0) {
        const dpr = C.DPR,
          cur = ctx.getImageData(0, 0, S.CW * dpr, S.CH * dpr),
          cd = cur.data;
        for (let y = 0; y < entry.h; y++)
          for (let x = 0; x < entry.w; x++) {
            const si = 4 * (y * entry.w + x),
              di = 4 * ((entry.y + y) * (S.CW * dpr) + (entry.x + x));
            ((cd[di] = patch[si]),
              (cd[di + 1] = patch[si + 1]),
              (cd[di + 2] = patch[si + 2]),
              (cd[di + 3] = patch[si + 3]));
      ...
      if (vPatch && G.VectorPaths) {
        ...
        (fm && fm.set(entry.l, JSON.parse(JSON.stringify(vPatch))),
  ...
  ((E.initWorker = () => {
    try {
      const url =
        (document.querySelector("script[data-worker-url]") || {}).dataset
          ?.workerUrl || "/static/js/workers/compute.js";
      ((worker = new Worker(url)),
        (worker.onmessage = (e) => {
          const d = e.data,
            cb = workerCbs.get(d.reqId);
          cb &&
            ("progress" === d.type
              ? cb.onProgress && cb.onProgress(d)
              : (workerCbs.delete(d.reqId), cb.resolve(d)));
        }));
    } catch (e) {
      worker = null;
    }
    ...
    (E.workerCall = (msg, transfers, onProgress) =>
      new Promise((resolve) => {
        const r = ++workerReqId;
        (workerCbs.set(r, { resolve: resolve, onProgress: onProgress }),
          (msg.reqId = r),
        ...
        if (i <= currentFrameIdx - threshold) {
          ...
          if (
            (["A", "B", "C"].forEach((layer) => {
              ...
              paths && paths.length > 0 && (hasVectors = !0);
            }),
            hasVectors)
          ) {
            ...
            (["A", "B", "C"].forEach((layer) => {
              ...
              paths &&
                ...
                c[layer] && tx.putImageData(c[layer], 0, 0),
                tx.save(),
                tx.scale(dpr, dpr),
              ...
              (S.frames[i].thumbDirty = !0));
          }
        }
      changed &&
        G.Timeline &&
        (G.Timeline.renderDebounced(), G.Storage && G.Storage.debounceSave());
    }),
    (G.Engine = E));
})(window.UgokuDraw);
