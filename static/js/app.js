...
if(S.ct==='text'){
    ...
    else if(S.ct==='eraser'){E.pushUndo();const dpr=C.DPR;S.eSnap=E.curX().getImageData(0,0,S.CW*dpr,S.CH*dpr);S.pts=[pt];T.applyEraser();}
  ...
  vp.addEventListener('pointermove',e=>{
    ...
    if(S.cpMode && S.cpDragging && VP){
      ...
      E.pushUndo();
      VP.rerasterize(E.curId(),S.cl,E.curX(),S.CW,S.CH,C.DPR);
      E.commitUndo();
    ...
    else if(S.ct==='eraser'){S.pts.push(pt);T.applyEraser();}
  ...
  function handlePointerUp(e){
    ...
    else if(S.ct==='eraser'){T.commitEraser();S.eSnap=null;S.pts=[];E.commitUndo();E.afterEdit();}
else if(['line','rect','circle','star','heart'].includes(S.ct)){
      const pt=R.screenToCanvas(e.clientX,e.clientY,S);
      const dpr=C.DPR;
            E.curX().save();
      E.curX().setTransform(1, 0, 0, 1, 0, 0);
      E.curX().drawImage(R.canvases.drC, 0, 0);
      E.curX().restore();
      E.curX().setTransform(dpr, 0, 0, dpr, 0, 0);
  ...
  vp.addEventListener('dblclick',()=>{if(S.img)T.commitImage();});
  ...
  U.$('selOutlineBtn').onclick=()=>T.selectionAction('outline');
  U.$('selOutWidth').oninput=e=>{U.$('selOutWidthLabel').textContent=e.target.value;};
  ...
  U.$('mergeDownBtn').onclick=()=>{
    ...
    E.pushUndo();E.getLayerCtx(S.layerOrder[idx-1]).drawImage(E.curC(),0,0);E.curX().clearRect(0,0,S.CW,S.CH);
    E.commitUndo();E.afterEdit();U.$('mergeModal').classList.remove('show');E.toast('下に結合しました');
  };
  U.$('mergeAllBtn').onclick=()=>{
    E.pushUndo();const tmp=document.createElement('canvas');tmp.width=S.CW;tmp.height=S.CH;const tc=tmp.getContext('2d');
  ...
  U.$('prevAudioBtn').onclick=()=>A.previewBGM(3);
  ...
  document.addEventListener('keydown',e=>{
    ...
    if(e.key==='Enter'&&S.img)T.commitImage();
    if(e.key==='Escape'){if(S.img)T.cancelImage();if(S.cpMode){S.cpMode=false;S.cpSelectedPath=null;R.contexts.cursorC.clearRect(0,0,S.CW,S.CH);U.$$('.dock-tool').forEach(b=>b.classList.toggle('on',b.dataset.tool==='pen'));S.ct='pen';}if(S.ct==='paste'){R.contexts.floatC.clearRect(0,0,S.CW,S.CH);S.ct='pen';U.$$('.dock-tool').forEach(b=>b.classList.toggle('on',b.dataset.tool==='pen'));}}
