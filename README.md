...
うごメモ風アニメーション作成ツールです。昔の「うごくメモ帳 for ブラウザ」を魔改造したものです。ベクター関連とバグ修正以外はAIにやらせています（ベクター関連はAIにできないと言われてしまったため）。
...
└── static/
    ...
    ├── sw.js             # Service Workerファイル（静的リソース用）
    ├── css/
    │   └── app.css       # メインスタイルシート
    ├── icons/            # PWA用アイコン画像
    └── js/
        ├── app.js        # アプリのエントリーポイント
        ├── sw.js         # Service Worker（fetch処理）
